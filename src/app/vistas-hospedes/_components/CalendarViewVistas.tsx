...
import { startOfMonth, endOfMonth, eachDayOfInterval, format, isSameMonth, addMonths, subMonths, getDay, isToday, parseISO } from 'date-fns';
...
import { ContentCardVistas } from './ContentCardVistas';
import { ContentModalVistas } from './ContentModalVistas';
import { QuickCreateModalVistas } from './QuickCreateModalVistas';
import type { Post, EditorialSlug, ContentStatus, ContentFormat } from '@/app/social-midia/calendario-seazone/_lib/types';
...
function DayModal({ date, items, onClose, onSelectItem, onStatusChange, onDuplicate, onUpdate }: DayModalProps) {
  const [d, m, y] = [parseInt(date.substring(8, 10)), parseInt(date.substring(5, 7)) - 1, parseInt(date.substring(0, 4))];
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
      <div style={{ width: '100%', maxWidth: 520, background: T.card, borderRadius: 14, padding: 24, boxShadow: T.elevMd, maxHeight: '85vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: T.cardFg, margin: '0 0 2px', textTransform: 'capitalize' }}>{displayDate}</h3>
            <p style={{ fontSize: 13, color: T.mutedFg, margin: 0 }}>{items.length} {items.length === 1 ? 'post' : 'posts'}</p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.cinza400 }}><X size={20} /></button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map((item) => (
            <ContentCardVistas key={item.id} item={item} compact={false}
              onClick={() => { onClose(); onSelectItem(item); }}
              onStatusChange={onStatusChange} onDuplicate={onDuplicate}
...
export function CalendarViewVistas() {
  ...
  const getItemsForDay = (day: Date) => {
    ...
    return filteredItems.filter((item) => item.scheduled_at && item.scheduled_at.substring(0, 10) === dayStr);
  };

  const showToast = useCallback((message: string) => { setToast(message); setTimeout(() => setToast(null), 2500); }, []);

  const handleDuplicate = async (item: Post) => {
    try {
      await createItem({ title: `${item.title} (copia)`, editoria: item.editoria, formato: item.formato, canal: item.canal, status: 'ideia', scheduled_at: item.scheduled_at, tema: item.tema, estrutura: item.estrutura, copy: item.copy, notas: item.notas });
      await fetchItems(); showToast('Post duplicado!');
    } catch { showToast('Erro ao duplicar'); }
  };

  const handleDragOver = (e: DragEvent<HTMLDivElement>, dateStr: string) => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setDragOverDate(dateStr); };
  ...
  const handleDrop = async (e: DragEvent<HTMLDivElement>, targetDate: string) => {
    ...
    try { await updateItem(itemId, { scheduled_at: targetDate }); await fetchItems(); showToast(`Post movido`); } catch { showToast('Erro ao mover'); }
  };

  const WEEKDAYS = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab'];
  const dayModalItems = useMemo(() => { if (!dayModalDate) return []; return filteredItems.filter((item) => item.scheduled_at && item.scheduled_at.substring(0, 10) === dayModalDate); }, [dayModalDate, filteredItems]);
  ...
  return (
    <div>
      ...
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, overflow: 'hidden' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 0', color: T.cinza400 }}>Carregando...</div>
        ) : (
            ...
            {days.map((day, i) => {
              ...
              return (
                <div key={i}
                  ...
                  style={{ height: 130, overflow: 'hidden', borderBottom: `1px solid ${T.cinza50}`, borderRight: `1px solid ${T.cinza50}`, padding: 8, transition: 'background 0.15s', background: isDragOver ? T.pendingBg : today ? `${T.pendingBg}80` : !isCurrentMonth ? `${T.cinza50}80` : 'transparent', boxShadow: isDragOver ? `inset 0 0 0 2px ${T.primary}60` : today ? `inset 0 0 0 2px ${T.primary}30` : 'none', position: 'relative' }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span onClick={() => isCurrentMonth && dayItems.length > 0 && setDayModalDate(dateStr)}
                      style={{ display: 'inline-flex', width: 24, height: 24, alignItems: 'center', justifyContent: 'center', borderRadius: '50%', fontSize: 12, fontWeight: today ? 700 : 500, background: today ? T.primary : 'transparent', color: today ? T.primaryFg : isCurrentMonth ? T.cinza700 : T.cinza200, flexShrink: 0, cursor: isCurrentMonth && dayItems.length > 0 ? 'pointer' : 'default' }}>
                      {format(day, 'd')}
                    </span>
                    {isCurrentMonth && (
                      <button onClick={(e) => { e.stopPropagation(); setQuickCreateDate(dateStr); }} title="Adicionar post"
                        style={{ background: 'none', border: `1px solid ${T.border}`, cursor: 'pointer', color: T.cinza400, padding: '1px 4px', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: isHovered ? 1 : 0, transition: 'opacity 0.15s' }}>
                        <Plus size={12} />
                      </button>
                    )}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3, overflow: 'hidden' }}>
                    {dayItems.slice(0, 2).map((item) => (
                      <ContentCardVistas key={item.id} item={item} compact draggable
                    ...
                    {dayItems.length > 2 && (
                      <button onClick={() => setDayModalDate(dateStr)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: T.primary, fontWeight: 600, padding: '1px 2px', textAlign: 'left' }}>
      ...
      {dayModalDate && (
        <DayModal date={dayModalDate} items={dayModalItems} onClose={() => setDayModalDate(null)}
      ...
      {selectedItem && (
        <ContentModalVistas item={selectedItem} onClose={() => setSelectedItem(null)} onUpdate={updateItem} onDelete={deleteItem} />
      )}

      {quickCreateDate && (
        <QuickCreateModalVistas date={quickCreateDate} onClose={() => setQuickCreateDate(null)}
          onCreate={async (item) => { const created = await createItem(item); showToast('Post criado!'); return created; }} />
      )}

      {toast && (
        <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 50, display: 'flex', alignItems: 'center', gap: 8, background: T.fg, borderRadius: 12, padding: '12px 20px', fontSize: 14, fontWeight: 500, color: T.primaryFg, boxShadow: T.elevMd }}>
          <CheckCircle size={18} color={T.statusOk} />{toast}
        </div>
      )}
    </div>
  );
