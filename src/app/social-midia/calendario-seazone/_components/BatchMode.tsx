...
export function BatchMode({ onNavigate }: BatchModeProps) {
  ...
  const [selectedFormats, setSelectedFormats] = useState<ContentFormat[]>(['carrossel']);
  ...
  const selectAllEditorias = () => {
    if (selectedEditorias.length === EDITORIALS.length) {
      setSelectedEditorias([]);
    } else {
      setSelectedEditorias(EDITORIALS.map((e) => e.slug));
    }
  };
  ...
  const plan = useMemo((): BatchItem[] => {
    ...
    for (let i = 0; i < postCount; i++) {
      ...
      const editorial = getEditorial(ed);
      const formatLabel = FORMATS.find((f) => f.value === fmt)?.label ?? fmt;

      const topic = generalTopic
        ? `${generalTopic} — foco na editoria ${editorial?.name}`
        : `Conteudo ${formatLabel} para a editoria ${editorial?.name}: ${editorial?.description}`;
  ...
  const handleGenerate = async () => {
    ...
    for (let i = 0; i < plan.length; i++) {
      ...
      try {
        ...
        batchResults.push({
          index: i, title: `Post ${i + 1}`, editorial: item.editorial, format: item.format,
  ...
  if (status === 'done') {
    return (
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 32, textAlign: 'center' }}>
        ...
        {failCount > 0 && (
          <p style={{ fontSize: 14, color: T.destructive, margin: '0 0 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 12 }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 8, background: T.primary, color: T.primaryFg, border: 'none', borderRadius: 12, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
          >
            <Calendar size={16} />
            Ver no Calendario
          </button>
          <button
            onClick={() => { setStatus('idle'); setResults([]); setCurrentIndex(0); }}
            style={{ background: 'none', border: `1px solid ${T.border}`, borderRadius: 12, padding: '10px 24px', fontSize: 14, fontWeight: 600, color: T.cinza600, cursor: 'pointer' }}
          >
            Criar mais
          </button>
        </div>
  ...
  if (status === 'generating') {
    ...
    const currentEd = getEditorial(currentItem?.editorial ?? 'inteligencia_mercado');
    const currentFmt = FORMATS.find((f) => f.value === currentItem?.format)?.label;
    ...
    return (
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 32 }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 700, color: T.cardFg, margin: '0 0 4px' }}>
            Gerando post {currentIndex + 1} de {plan.length}...
          </h2>
  ...
  return (
    <>
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24, marginBottom: 24 }}>
        <div style={{ marginBottom: 20 }}>
          ...
          <input
            ...
            max={20}
            value={postCount}
            onChange={(e) => setPostCount(Math.min(20, Math.max(1, Number(e.target.value))))}
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <button onClick={selectAllEditorias} style={{ background: 'none', border: 'none', fontSize: 12, fontWeight: 500, color: T.primary, cursor: 'pointer' }}>
              {selectedEditorias.length === EDITORIALS.length ? 'Desmarcar todas' : 'Selecionar todas'}
            </button>
          </div>
            ...
            {EDITORIALS.map((e) => {
              ...
              return (
                <button
                  ...
                  style={{
                    border: `2px solid ${selected ? e.color : T.border}`,
                    borderRadius: 12, padding: 12, textAlign: 'left', cursor: 'pointer',
                    background: T.card, opacity: selected ? 1 : 0.5,
                    boxShadow: selected ? T.elevSm : 'none', transition: 'all 0.15s',
                  }}
                >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <textarea
            ...
            placeholder='Ex: "Conteudo para a semana de alta temporada em Floripa"'
            rows={2}
      ...
      {plan.length > 0 && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24, marginBottom: 24 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: T.cardFg, margin: '0 0 16px' }}>
            Preview — {plan.length} {plan.length === 1 ? 'post' : 'posts'}
          </h3>
          <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, overflow: 'hidden' }}>
            <table style={{ width: '100%', textAlign: 'left', fontSize: 14, borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {plan.map((item, i) => {
                  const ed = getEditorial(item.editorial);
                  const fmtLabel = FORMATS.find((f) => f.value === item.format)?.label;
                  return (
          ...
          <button
            ...
            disabled={selectedEditorias.length === 0 || selectedFormats.length === 0}
            style={{
              ...
              opacity: (selectedEditorias.length === 0 || selectedFormats.length === 0) ? 0.5 : 1,
            }}
          >
            <Sparkles size={18} />
            Gerar todos com IA
