...
function StoryModal({ date, initial, onClose, onSave, mode }: StoryModalProps) {
  ...
  const handleSave = async () => {
    if (!name.trim()) { setError('Nome do story e obrigatorio'); return; }
    setSaving(true);
    try {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: T.card, borderRadius: 16, padding: 28, width: 460,
        maxHeight: '90vh', overflowY: 'auto',
        boxShadow: T.elevMd, border: `1px solid ${T.border}`,
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: T.cardFg, margin: 0 }}>
              {mode === 'add' ? 'Adicionar Story' : 'Editar Story'}
            </h3>
            <p style={{ fontSize: 12, color: T.mutedFg, margin: '2px 0 0' }}>{displayDateStr(date)}</p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.mutedFg }}>
            <X size={18} />
          </button>
        </div>

        <div style={{ marginBottom: 20 }}>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: T.cinza700, marginBottom: 6 }}>
            Nome do story *
          </label>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Ex: Promocao verao - desconto 20%"
            autoFocus
            style={{
              width: '100%', border: `1px solid ${T.border}`, borderRadius: 10,
              padding: '10px 14px', fontSize: 14, color: T.cardFg, background: T.muted,
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button onClick={addLink} style={{
              ...
              padding: '4px 10px', fontSize: 12, color: T.mutedFg, cursor: 'pointer',
            }}>
            ...
            {links.map((link, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6, background: T.muted, borderRadius: 10, padding: 12, border: `1px solid ${T.border}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {links.length > 1 && (
                    <button onClick={() => removeLink(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.cinza400, padding: 2 }}>
                      <X size={12} />
                    </button>
                  )}
                </div>
                <input value={link.name} onChange={(e) => updateLink(i, 'name', e.target.value)}
                  placeholder="Nome do link (ex: Site, Linktree, WhatsApp)" style={inputStyle} />
        ...
        {error && (
          <div style={{ marginBottom: 16, background: T.statusErrBg, borderRadius: 8, padding: '8px 12px', fontSize: 13, color: T.destructive }}>
            {error}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{
            ...
            padding: '9px 20px', fontSize: 13, fontWeight: 500, color: T.mutedFg, cursor: 'pointer',
          }}>
            Cancelar
          </button>
          <button onClick={handleSave} disabled={saving} style={{
            background: T.primary, border: 'none', borderRadius: 10,
            padding: '9px 20px', fontSize: 13, fontWeight: 600, color: T.primaryFg,
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedDate ? '1fr 360px' : '1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <button onClick={prevMonth} style={{ background: 'none', border: `1px solid ${T.border}`, borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: T.mutedFg }}>
              <ChevronLeft size={16} />
            </button>
            <span style={{ fontSize: 16, fontWeight: 700, color: T.cardFg }}>{MONTHS_PT[month]} {year}</span>
            <button onClick={nextMonth} style={{ background: 'none', border: `1px solid ${T.border}`, borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: T.mutedFg }}>
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    color: isToday ? T.primary : T.cardFg,
                    marginBottom: 4, width: 22, height: 22, borderRadius: '50%',
                    background: isToday ? T.pendingBg : 'transparent',
                  ...
                  {dateStr && (
                    <button
                      ...
                      style={{
                        ...
                        color: T.primaryFg, padding: 2, borderRadius: 4,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <style>{`div:hover > .cell-add-btn { opacity: 1 !important; }`}</style>
        </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: '6px 12px', fontSize: 12, fontWeight: 600, color: T.primaryFg, cursor: 'pointer',
                    }}
                  >
                    <Plus size={14} />
                    Adicionar
                  </button>
                  <button onClick={() => setSelectedDate(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.mutedFg }}>
                    <X size={16} />
                  </button>
                </div>
              </div>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ height: 6, borderRadius: 3, background: T.cinza50, overflow: 'hidden' }}>
                    <div style={{
                      ...
                      background: done === total ? T.statusOkFg : T.primary,
                ...
                <div style={{ textAlign: 'center', padding: '28px 0' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: T.primary, border: 'none', borderRadius: 8,
                      padding: '8px 16px', fontSize: 13, fontWeight: 600, color: T.primaryFg, cursor: 'pointer',
                    }}
                  >
                    <Plus size={14} />
                    Adicionar story
                              ...
                              {story.links.map((link, idx) => (
                                <a
                                  ...
                                  style={{
                                    ...
                                    background: T.card, borderRadius: 6, padding: '3px 8px',
      ...
      {editingStory && (
        <StoryModal
          ...
          initial={{ name: editingStory.name, links: editingStory.links }}
